...
const Notification: React.FC<NotificationProps> = ({ logo, title, datetime, content, tooltipContent }) => {
  // const IconComponent = iconMap[logo.icon] || Info
  const IconComponent = logo && logo.icon ? iconMap[logo.icon] : Info;
  const bgColor = logo && logo.bgColor ? logo.bgColor : 'bg-blue-500'

  return (
    <div className="flex items-center justify-between p-2 hover:bg-gray-100 rounded-lg">
      <div className="flex items-center space-x-3">
        <div className={`w-8 h-8 rounded flex items-center justify-center text-white shadow ${bgColor}`}>
